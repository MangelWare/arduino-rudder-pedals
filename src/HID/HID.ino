...
void loop() {
  // Center signal
  c_value = get_calibrated_c(analogRead(A_PIN_C));
  Gamepad.xAxis(c_value);
  
  // Left signal
  l_value = get_calibrated_l(analogRead(A_PIN_L));
  Gamepad.rxAxis(l_value);
  
  // Right signal
  r_value = get_calibrated_r(analogRead(A_PIN_R));
  Gamepad.ryAxis(r_value);

  Gamepad.write();

  if(button_c == 0 && !digitalRead(resetButton)) {
    reset_calib();
    button_c = 100;
  }
  if(button_c >0) {
